...
def getArgs():
    ...
    parser.add_argument('--lite', '-lt', help='Run in Lite mode',
                        required=False, action='store_true')
    parser.add_argument(
        '--dev', '-d', help='ETM dev mode. Usage: --dev=etm', required=False)
    parser.add_argument('--forcepull', '-fp', help='Force pull of all images. Usage: --forcepull',
                        required=False, action='store_true')
    ...
    parser.add_argument('--logs', '-l', help='Show logs of all containers. Usage: --logs',
                        required=False, action='store_true')
    parser.add_argument('--wait', '-w', help='Wait until etm has been started. Usage: --wait',
                        required=False, action='store_true')

    # Custom usage message
    usage = parser.format_usage()
    usage = usage.replace(
        "usage: run.py", "docker run -v /var/run/docker.sock:/var/run/docker.sock --rm elastest/platform")
    parser.usage = usage
...
# If is NORMAL mode
if(lite == False):
    dockerCommand = 'docker-compose ' + platform_services + ' ' + edm + ' ' + \
        etm + ' ' + esm + ' ' + eim + ' ' + epm + ' ' + emp + ' -p elastest'
