...
def startK8(args):
    ...
    # Start EPM
    startEpmCommand = "docker-compose -f ../epm/deploy/docker-compose.yml up -d"
    result = subprocess.call(shlex.split(startEpmCommand), stderr=FNULL)

    if(result == 0):
        # STEP 1: REPLACE HERE WITH THE EPM IP !!!
        api_client = ApiClient(
            host="http://" + epm_service_name + ":" + epm_port + "/v1")
